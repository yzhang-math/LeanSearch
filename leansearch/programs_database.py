...
class ProgramsDatabase:
  ...
  def get_prompt(self) -> Prompt:
    ...
    #logging.info(f"Database getting prompt from island {island_id}")
    #print("Getting prompt from island: ", island_id)
    code, version_generated = self._islands[island_id].get_prompt()
    island_version = self._islands[island_id]._island_version
    #print("Prompt: ", code)
    #logging.info(f"Database got prompt from island {island_id} with version {island_version}")
    if '--@funsearch.evolve' in code:
      code = re.sub('--@funsearch.evolve','',code)
      #logging.info("Removed evolve comment from prompt")
...
class Island:
  ...
  def get_prompt(self) -> tuple[str, int]:
    ...
    #logging.info(f"To generate prompt in island")
  ...
  def _generate_prompt(
    ...
    #logging.info(f"Generating prompt: step 1")
    
    versioned_functions: list[code_manipulation.Function] = []
    for i, implementation in enumerate(implementations):
      #logging.info(f'found imple {str(implementation)}')
    ...
    #logging.info(f"Generating prompt: step 3")
    # Replace functions in the template with the list constructed here.
    prompt = dataclasses.replace(self._template, functions=versioned_functions)
    #logging.info(f"Generating prompt: step 4")
    return str(prompt)
